...
export default function AdminContactsPage() {
  ...
  const fetchContacts = async (opts: { forceToast?: boolean } = {}) => {
    ...
    try {
      ...
      const contacts_array = response_data.data || response_data;
  ...
  const filteredContacts = contacts.filter((contact: Contact) => {
    ...
    const matchesSubject = contact.subject?.toLowerCase()?.includes(q) ?? false;
    const matchesPhone = contact.phone?.toString()?.includes(q) ?? false;
    const matchesInquiryType = contact.inquiryType?.toLowerCase()?.includes(q) ?? false;
    const matchesSearch = matchesName || matchesEmail || matchesSubject || matchesPhone || matchesInquiryType;
  ...
  const updateStatus = async (id: string, newStatus: "new" | "in-progress" | "resolved" | "closed") => {
    try {
                          ...
                          <div className="flex gap-2 pt-2 border-t border-amber-200">
                            <button
                              onClick={() => {
                                const nextStatus = 
                                  ...
                                  contact.status === "in-progress" ? "resolved" :
                                  contact.status === "resolved" ? "closed" : "new";
                                updateStatus(cid, nextStatus);
                              }}
                            ...
                            >
                              ...
                              <span className="text-xs">
                                {contact.status === "new" ? "Start" :
                                 contact.status === "in-progress" ? "Resolve" :
                                 contact.status === "resolved" ? "Close" : "Reopen"}
                              </span>
                            </button>
                            <button
                          ...
                          >
                            ...
                            <td className="px-4 py-3">
                              ...
                              >
                                ...
                                ></div>
                                {contact.status.toUpperCase().replace("-", " ")}
                              </span>
                            </td>
                            ...
                            <td className="px-4 py-3">
                              <div className="flex items-center justify-center gap-2">
                                <button
                                  ...
                                  title={
                                    contact.status === "new" ? "Start Working" :
                                    contact.status === "in-progress" ? "Mark Resolved" :
                                    contact.status === "resolved" ? "Close" : "Reopen"
                                  }
                                >
                                  {contact.status === "resolved" || contact.status === "closed" ? (
